...
const FormikTimeRadioButtonsWrapper: FC<CustomInputComponent & FieldProps> = ({ field,
    form,
    ...props }) => {
    const [availableTime, setAvailableTime] = useState<FreeTime>([]);

    const { refetch } =
        useQuery<FreeTime, Error>('freeTime', () => getDoctorsFreeTimeByDateAndId(form.values.selectedDate, form.values.doctor.value),
            {
...
export default FormikTimeRadioButtonsWrapper;